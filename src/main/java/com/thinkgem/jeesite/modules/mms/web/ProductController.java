...
public class ProductController extends BaseController {
     ...
     * 回复意见时间，剩30天	 下意见时间选择后显示，倒计时			超过15天后变为红色叹号标记
     * 办理批件延续  取得批件时间选择后加3个自然年，超过此日期后显示。
     * <p>
     * 这个是有先后优先级的  从后向前进行判断
     *
     * @param productList
    ...
    private List<Product> handleDateLimitContent(List<Product> productList) {
        ...
        for (Product product : productList) {
            ...
            Date documentTime = product.getDeclareProduct().getDocumentTime();//批件时间


            double dateBetween = 0L; //时间间隔

            if (documentTime != null) {
                if (now.getYear() > (documentTime.getYear() + 3)) {
                    product.setDateLimitContent("办理批件延续");
                }
            } else {
                //第一种情况
                if (nextOpinionTime != null && replyOpinion == null) { //下意见时间 有，回复意见没有
                    ...
                    if (dateBetween > 15) {
                        product.setDateLimitContent("回复意见时间，剩30天 <font color='red'>!</font>");
                    } else {
                        product.setDateLimitContent("回复意见时间，剩" + (30 - dateBetween) + "天");
                    }
                } else if (sendRiskTestTime == null) { //第二种情况
                ...
                } else if (administrativeLicenseInspectionTime == null && sampleTime != null) { //第3种情况
    ...
    public String productInformation(Product product, HttpServletRequest request, HttpServletResponse response, Model model, RedirectAttributes redirectAttributes) {
        ...
        if (StringUtils.isNoneEmpty(isFirst) && isFirst.equals("1")) { //首次进入，需要赋默认显示项
            ...
            product.setIsShowCountryOfOrigin("1");
            product.setIsShowProductType("1");
            product.setIsShowWorkMatters("1");
            product.setIsShowProductLeader("1");
            product.setIsShowProjectLeader("1");
            product.setIsShowEnterpriseApplication("1");
            product.setIsShowEnterpriseApplicationAddress("1");
            product.setIsShowEnterpriseApplicationPhone("1");
            product.setIsShowEnterpriseApplicationContacts("1");
            product.setIsShowActualProductionEnterprise("1");
            product.setIsShowActualProductionEnterpriseAddress("1");
            product.setIsShowResponsibleUnitInChina("1");
            product.setIsShowResponsibleUnitInChinaAddress("1");
            product.setIsShowResponsibleUnitInChinaPhone("1");
            product.setIsShowResponsibleUnitInChinaFax("1");
            product.setIsShowResponsibleUnitInChinaZipCode("1");
            product.setIsShowProjectTime("1");
            product.setIsShowContractNumber("1");
            product.setIsShowContractSigningTime("1");
            product.setIsShowArrivalCompany("1");
            product.setIsShowArrivalTime("1");
            product.setIsShowSampleTime("1");
            product.setIsShowSampleQuantity("1");
            product.setIsShowTotalNumberOfSamples("1");
            product.setIsShowColorCharacter("1");
            product.setIsShowSampleMarking("1");
            product.setIsShowDateOfExpiry("1");
            product.setIsShowTechnologyDateOfExpiry("1");
            product.setIsShowSmell("1");
            product.setIsShowSpecifications("1");
            product.setIsShowAdministrativeLicenseInspectionTime("1");
            product.setIsShowAdministrativeLicenseInspectionOrganization("1");
            product.setIsShowAdministrativeLicenseInspectionProject("1");
            product.setIsShowAdministrativeLicenseInspectionNumber("1");
            product.setIsShowAdministrativeLicenseInspectionNo("1");
            product.setIsShowAdministrativeLicenseInspectionReportTime("1");
            product.setIsShowSendBodyTime("1");
            product.setIsShowSendBodyOrganization("1");
            product.setIsShowSendBodyProject("1");
            product.setIsShowSendBodyNumber("1");
            product.setIsShowHumanTestAcceptanceNo("1");
            product.setIsShowHumanTestAcceptanceReportTime("1");
            product.setIsShowSendRiskTestTime("1");
            product.setIsShowSendRiskTestOrganization("1");
            product.setIsShowSendRiskTestProjectr("1");
            product.setIsShowSendRiskTestNumber("1");
            product.setIsShowRiskTestAcceptanceNo("1");
            product.setIsShowRiskTestAcceptanceReportTime("1");
            product.setIsShowReportTime("1");
            product.setIsShowAcceptanceTime("1");
            product.setIsShowAcceptanceNumber("1");
            product.setIsShowDocumentTime("1");
            product.setIsShowDocumentNumber("1");
            product.setIsShowProductStatusRemark("1");
            product.setIsShowNextOpinionTime("1");
            product.setIsShowNextOpinionTime("1");
            product.setIsShowOpinionContent("1");
            product.setIsShowReplyOpinion("1");
            product.setIsShowOtherDescription("1");
        }
        model.addAttribute("page", page);
